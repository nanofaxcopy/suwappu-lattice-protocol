...
[`RED_TEAM_CAMPAIGN_2026-05.md`](RED_TEAM_CAMPAIGN_2026-05.md)
and [`security/campaigns/`](../threat-intel/).

| Hack | Year | Loss | Pattern | LTP exposure | Scenario |
|---|---|---|---|---|---|
| Wormhole | 2022 | $326M | Off-by-one in guardian-set signature verification | `LTP-A-001` | [SCN-001](../threat-intel/SCN-001-wormhole-signature-skip/) |
| Nomad | 2022 | $190M | Zero-hash treated as auto-confirmed root after upgrade | `LTP-A-003` (defense confirmed) | [SCN-002](../threat-intel/SCN-002-nomad-init-bug/) |
| Ronin | 2022 | $625M | 5-of-9 validator threshold; 5 keys compromised | `LTP-A-002` (worse: 2-of-2) | [SCN-008](../threat-intel/SCN-008-ronin-active-set/) |
| Multichain (Anyswap) | 2023 | $125M | Single-custody MPC key | `LTP-A-004` | [SCN-012](../threat-intel/SCN-012-multichain-single-custody/) |
| Poly Network | 2021 | $600M | `putCurEpochConPubKeyBytes` cross-contract delegatecall hijack | `LTP-A-005` | [SCN-003](../threat-intel/SCN-003-poly-keeper-escalation/) |
| Harmony Horizon | 2022 | $100M | 2-of-5 MultiSig; 2 keys compromised | `LTP-A-002` | [SCN-009](../threat-intel/SCN-009-harmony-low-threshold/) |
| Orbit Chain | 2024 | $82M | Compromised signer keys + weak replay protection | `LTP-A-008` (defense confirmed) | [SCN-004](../threat-intel/SCN-004-orbit-multisig-subversion/) |
| Cypher Protocol | 2023 | $1M | Proxy admin compromised during upgrade window | `LTP-A-009` | [SCN-016](../threat-intel/SCN-016-cypher-pause-bypass/) + [SCN-019](../threat-intel/SCN-019-timelock-floor/) |
| LayerZero DVN debate | 2024 | (none yet) | Same operator runs relayer + oracle = collusion | `LTP-A-006` | [SCN-017](../threat-intel/SCN-017-layerzero-dvn/) |
| KyberSlash | 2024 | (academic) | ML-KEM error-correction timing leak | `LTP-A-014` | (covered by audit suite directly) |
| Signer-rotation grace race | (campaign-surfaced) | n/a | `_anchor` ignored `signerExpiresAt` | **`LTP-A-031`** (REMEDIATED) | [SCN-015](../threat-intel/SCN-015-signer-rotation-grace/) |
...
**Severity.** HIGH.
**File.** `docs/security/audits/internal/001-lattice-key-shard-exposure.md`, `src/ltp/corridor/envelope.py`.
...
The pre-existing `docs/security/audits/internal/001-lattice-key-shard-exposure.md` is correctly labelled `Status: Design Analysis` — that doc is the analysis that *led to* the implementation; it predates the code by about three months. The audit row is updated to point at the actual implementation. Regression-guarded by the existing protocol test suite.
...
- `docs/security/audits/internal/SECURITY_REVIEW-2-24-2026.md` — formal security review
