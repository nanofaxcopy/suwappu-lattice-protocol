# SUWAPPU Stack Visuals

Diagrams of the LTP transfer flow, the SUWAPPU stack it sits inside, and the protocol details that auditors and reviewers tend to ask about. Every inline Mermaid block renders natively on GitHub and GitBook (no plugin). Bare sources live under `mermaid/` and `excalidraw/` for remixing in [Mermaid Live](https://mermaid.live) or [Excalidraw](https://excalidraw.com).
...
| 4 | [SUWAPPU DAG — Layer 1](#suwappu-dag--layer-1) | architect | flowchart |
| 5 | [SUWAPPU DB — Canonical State Substrate](#suwappu-db--canonical-state-substrate) | architect | flowchart |
...
| 11 | [SUWAPPU Ecosystem Atlas](#suwappu-ecosystem-atlas) | exec summary | HTML deck |
...
The DAG L1 owns certificate-DAG ordering (Mysticeti-C), validator-ring consensus, and dual-VM execution. The Authority Ring proposes blocks; the Validator Ring votes and certifies; together they secure consensus. LTP corridor super-nodes attest the state roots that SUWAPPU-DB emits after DAG ordering.
...
- Standalone presentation: [`suwappu-dag.html`](./suwappu-dag.html)
- Mermaid source: [`mermaid/suwappu-dag.md`](./mermaid/suwappu-dag.md)
- Excalidraw source: [`excalidraw/suwappu-dag.excalidraw`](./excalidraw/suwappu-dag.excalidraw)

## SUWAPPU DB — Canonical State Substrate

SUWAPPU-DB owns capability-gated state mutation. The `suwappudb-lane` ingest is untrusted and **cannot import** `suwappudb-state` directly — every write goes through the `suwappudb-bridge` validator under optimistic concurrency control (OCC). Read projections (EVM `balanceOf`, Move `Coin::value`) are pure, read-only views off the canonical lattice. Replay/recovery is a separate, idempotent path off the same state — it does not mutate.

```mermaid
flowchart LR
  Lane[suwappudb-lane<br/>untrusted ingest] --> Bridge[suwappudb-bridge<br/>validator]
  Bridge -->|OCC validate| State[suwappudb-state<br/>canonical state]
...
- Standalone presentation: [`suwappu-db.html`](./suwappu-db.html)
- Mermaid source: [`mermaid/suwappu-db.md`](./mermaid/suwappu-db.md)
- Excalidraw source: [`excalidraw/suwappu-db.excalidraw`](./excalidraw/suwappu-db.excalidraw)

## End-to-end anchor lifecycle

How a SUWAPPU-DB state root becomes a verifiable on-chain anchor on a base chain. The `suwappudb-bridge` emits an anchor, the LTP corridor super-nodes form a 7-of-9 BLS attestation under `BLS_CORRIDOR_DST`, the aggregated signature is submitted to `LTPAnchorRegistry` on the destination chain, and any verifier can re-derive the digest.
...
- Source contracts: [`contracts/src/LTPAnchorRegistry.sol`](../../contracts/src/LTPAnchorRegistry.sol) (per-entity); `suwappu-db/contracts/src/LTPAnchorRegistry.sol` (per-chain)
- Disambiguation table: [`design-decisions/SUWAPPU_DAG_DB_INTEGRATION.md`](../design-decisions/SUWAPPU_DAG_DB_INTEGRATION.md)
...
- DST constant: [`src/ltp/corridor/constants.py`](../../src/ltp/corridor/constants.py) (matches `suwappu-dag/crates/suwappu-crypto/src/bls.rs::BLS_DST`)
- Mermaid source: [`mermaid/corridor-quorum.md`](./mermaid/corridor-quorum.md)

## SUWAPPU Ecosystem Atlas

Big-picture map of how LTP, SUWAPPU DAG, and SUWAPPU-DB compose into a settlement-grade stack.

- Standalone presentation: [`suwappu-ecosystem-atlas.html`](./suwappu-ecosystem-atlas.html)
- Visual index entry point: [`index.html`](./index.html)

## Editing

The inline Mermaid blocks in this file are the **canonical source going forward**. The standalone HTML decks (`ltp.html`, `suwappu-dag.html`, `suwappu-db.html`, `suwappu-ecosystem-atlas.html`, `index.html`) are historical snapshots from the initial deck export and are not auto-regenerated — if a diagram drifts, update the inline Mermaid here and the bare source under `mermaid/`, and only regenerate the HTML if the styled deck view is being actively used.
...
3. If the diagram semantics changed (not just labels), also update [`design-decisions/SUWAPPU_DAG_DB_INTEGRATION.md`](../design-decisions/SUWAPPU_DAG_DB_INTEGRATION.md) and any whitepaper or architecture section that references the same model.
