...
class HTTPFederationTransport(FederationTransport):
    ...
    def _get_client(self):
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "httpx is required for HTTP federation transport: "
                    "pip install httpx"
                )
        return self._client
    ...
    def _auth_headers(self, auth: FederationAuth) -> dict[str, str]:
        ...
        if hasattr(nir, "signature") and nir.signature:
            headers["X-Federation-NIR-Sig"] = nir.signature.hex() if isinstance(nir.signature, bytes) else str(nir.signature)
        ...
        if hasattr(agreement, "initiator_signature") and agreement.initiator_signature:
            init_sig = agreement.initiator_signature.hex() if isinstance(agreement.initiator_signature, bytes) else str(agreement.initiator_signature)
            resp_sig = ""
            if hasattr(agreement, "responder_signature") and agreement.responder_signature:
                resp_sig = agreement.responder_signature.hex() if isinstance(agreement.responder_signature, bytes) else str(agreement.responder_signature)
