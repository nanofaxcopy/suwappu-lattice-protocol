"""Bridge a `CorridorAttestation` into a gsx-db `StateAnchor`.

The 7-of-9 corridor attestation produces a quorum signature over an
`(source_chain, source_height, state_root, …)` payload. The gsx-db anchor
...
3. Returns a `StateAnchor` ready to submit to either the Rust
   `gsxdb-bridge` dispatcher or the Solidity `LTPAnchorRegistry.acceptAnchor`.
...
def build_state_anchor_blake3(
    ...
    """Project a corridor attestation onto a gsx-db `StateAnchor` using the
    ...
    if payload.target_chain >= (1 << 32):
        raise CorridorAnchorError(
            ...
            f"the gsx-db anchor registry keys chains by u32"
        )
...
def build_state_anchor_keccak256(
    ...
    has not yet swapped in the BLAKE3 precompile (pre-gsx-db sprint S11).
    """
    payload = attestation.payload
    if payload.target_chain >= (1 << 32):
        raise CorridorAnchorError(
